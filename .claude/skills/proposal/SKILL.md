...
1. **检查序列号**：查看 `.proposal/` 目录下现有提案，获取最大序列号
2. **计算新序号**：新序号 = 最大序号 + 1（如果没有则为 1）
3. **格式**：三位数字（001, 002, 003...）
4. **创建目录**：

```bash
# 示例：现有 001-user-auth, 002-order-query，下一个为 003
ls .proposal/  # 查看现有提案序号
mkdir -p .proposal/{序号}-{feature-name}
# 例如：mkdir -p .proposal/003-wechat-payment
```
...
👉 下一步:
   1. 检查文档，确认待确认项
   2. 执行 /apply {序号}-{feature-name} 开始实现
```
